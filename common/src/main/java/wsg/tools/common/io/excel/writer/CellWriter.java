...
public class CellWriter<V> {
     ...
     *
     * @return true if the value is null or the type of the value is one of String, Boolean,
     * LocalDate, LocalDateTime, Double, Date, Calendar, or RichTextString.
     */
    private static boolean setCellValue(Cell cell, Object value, Class<?> clazz) {
        if (value == null) {
            return true;
        }
    ...
    /**
     * Set val of the given cell.
     * <p>
     * If type of the val is one of those mentioned in {@link #setCellValue(Cell, Object, Class)},
     * it'll set directly. If not, the val will be convert to the target type, and then try to set
     * the cell again. If still not, it will throw an exception.
     *
     * @param cell   given cell
     * @param val    object to write
     * @param mapper mapper to convert values
     */
    public void setCellValue(Cell cell, V val, ObjectMapper mapper) {
        if (val == null) {
            return;
        }
        if (setCellValue(cell, val, val.getClass())) {
            return;
        }

        if (targetType != null) {
            Object targetValue =
                (ClassUtils.isAssignable(val.getClass(), targetType)) ? targetType.cast(val)
                    : mapper.convertValue(val, targetType);
            if (setCellValue(cell, targetValue, targetType)) {
                return;
            }
            throw new IllegalArgumentException("Can't write type of " + targetType + " to a cell.");
        }
        throw new IllegalArgumentException("Can't write type of " + val.getClass() + " to a cell.");
    }
}

...
public class MidnightSite extends BaseSite {
    ...
    private static final Pattern IMG_FILE_HREF_REGEX = Pattern.compile("/d/file/\\d{4}-\\d{2}-\\d{2}/[a-z0-9]{32}\\.(jpg|gif)");
    private static final Map<MidnightType, Pattern> ITEM_URL_REGEXES = Arrays.stream(MidnightType.values()).collect(Collectors.toMap(
            type -> type,
            type -> Pattern.compile("https://www\\.shenyequ\\.com/" + type.getText() + "/(?<id>\\d+).html")
    ));
    ...
    public synchronized static MidnightSite getInstance() throws HttpResponseException {
        if (instance == null) {
    ...
    public IterableRepository<MidnightWrapper<MidnightActress>> getActressRepository() throws HttpResponseException {
        return new IdentifiedIterableRepositoryImpl<>(this::findActress, getIdentifierIterator(MidnightType.ACTRESS));
    }

    public IterableRepository<MidnightWrapper<MidnightAlbum>> getAlbumRepository() throws HttpResponseException {
        return new IdentifiedIterableRepositoryImpl<>(this::findAlbum, getIdentifierIterator(MidnightType.ALBUM));
    }

    public IterableRepository<MidnightWrapper<MidnightEntry>> getEntryRepository(@Nonnull MidnightEntryType type) throws HttpResponseException {
        return new IdentifiedIterableRepositoryImpl<>(integer -> findAdultEntry(type, integer), getIdentifierIterator(type.getType()));
    }
    ...
    public MidnightWrapper<MidnightActress> findActress(int id) throws HttpResponseException {
        return initWrapper(MidnightType.ACTRESS, id, (contents, title) -> {
            ...
            for (Element content : contents) {
                Element current = content.selectFirst("nav.navigation").previousElementSibling().previousElementSibling();
                while (current != null) {
                    ...
                    for (Element img : elements) {
                        ...
                        Matcher matcher = IMG_FILE_HREF_REGEX.matcher(img.attr(CssSelector.ATTR_SRC));
    ...
    public MidnightWrapper<MidnightEntry> findAdultEntry(@Nonnull MidnightEntryType type, int id) throws HttpResponseException {
        return initWrapper(type.getType(), id, (contents, title) -> {
            List<String> images = Objects.requireNonNull(getImages(contents));
            Node current = contents.get(0).selectFirst("nav.navigation").previousElementSibling().previousSibling();
            List<String> texts = new ArrayList<>();
            for (; current != null; current = current.previousSibling()) {
                collectTexts(current, texts);
            }
            Map<String, String> info = texts.stream().map(s -> StringUtils.split(s, ":：", 2)).filter(ss -> ss.length > 1)
                    .collect(Collectors.toMap(ss -> {
    ...
    private <T extends TitleSupplier> MidnightWrapper<T>
    initWrapper(@Nonnull MidnightType type, int id, @Nonnull BiFunction<List<Element>, String, T> getContent) throws HttpResponseException {
        Document document = getDocument(builder0("/%s/%d.html", type.getText(), id), SnapshotStrategy.never());
        ...
        String keywords = document.selectFirst("meta[name=keywords]").attr(CssSelector.ATTR_CONTENT);
    ...
    private List<Integer> getIdentifierIterator(MidnightType type) throws HttpResponseException {
        ...
        RequestBuilder builder = builder0("/e/action/ListInfo.php")
                ...
                .addParameter("orderby", "newstime")
                .addParameter("line", "80");
        while (true) {
            Document document = getDocument(builder.setParameter("page", String.valueOf(page++)), SnapshotStrategy.always());
            ...
            for (Element li : lis) {
                ...
                ids.add(Integer.parseInt(RegexUtils.matchesOrElseThrow(ITEM_URL_REGEXES.get(type), href).group("id")));
            }

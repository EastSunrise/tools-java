 ...
 * @since 2020/12/6
 */
public class YyetsResource extends AbstractResource implements FilenameSupplier, LengthSupplier {
    ...
    public static YyetsResource of(String title, String url) throws InvalidResourceException {
        if (!StringUtils.startsWithIgnoreCase(url, YYETS_PREFIX)) {
            throw new UnknownResourceException(YyetsResource.class, title, url);
        }
        Matcher matcher = YYETS_REGEX.matcher(url);
        if (!matcher.matches()) {
            throw new InvalidResourceException(YyetsResource.class, title, url);
        }
